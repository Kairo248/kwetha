...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const payload = (await request.json().catch(() => null)) as { featured?: unknown } | null;

  if (!payload || typeof payload.featured !== "boolean") {
    return NextResponse.json({ message: "A valid featured value is required." }, { status: 400 });
  }
